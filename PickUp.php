<?php
	...
	if ($_SERVER['REQUEST_METHOD'] == 'POST') {
      	...
      	if (mysql_num_rows($result) > 0) {
		...
		} elseif (isset($_POST['btnCompletePickup'])) {
			if (empty($_POST['ccNumber']) or empty($_POST['expDate']) or empty($_POST['reservationNum'])) {
				$errorMsg = "Please fill in the reservation number, credit card number, and expiration date.";
			} else {
				/* Update the reservation with the credit card number, expiration, and pick-up clerk login */
				session_start();
    		$login = $_SESSION['login'];
    	
    		$expDate = $_POST['expDate'] . '-00';  /* store expiration date with 00 day */
    		...
    		$resNum = $_POST['reservationNum'];
				...
				$query = "SELECT
									...
									WHERE
											Number = $resNum
											AND PickupClerkLogin IS NULL
											AND CreditCardNumber IS NULL
											AND CreditCardExpiration IS NULL";
				
				$result = mysql_query($query);
    		
    		if(mysql_num_rows($result) == 0) {
    			$errorMsg = "Reservation number does not exist or reservation has already been picked up.";
    			mysql_close($connect);
    		} else {
    			
					$query2 = "UPDATE reservation
					SET PickupClerkLogin = '$login',
							CreditCardNumber = $CCNum,
							CreditCardExpiration = '$expDate'
					WHERE Number = $resNum
							AND PickupClerkLogin IS NULL
							AND CreditCardNumber IS NULL
							AND CreditCardExpiration IS NULL";
					...
					if(!$result2) {
						...
						/* Put the reservation number in session and redirect to the RentalContract page */
						mysql_close($connect);
...
<body style="text-align:center;">
	...
	<input type="text" name="reservationNum" class="pickup_input" /> <button type="submit" name="btnReservatonLookup" class="clerkMainMenu">Lookup</button>
	<br>
	<br>
		...
		if ($_SERVER['REQUEST_METHOD'] == 'POST') {
			if (isset($_POST['btnReservatonLookup'])) {
		        ...
		        if (mysql_num_rows($result2) > 0) {
							echo "<table style='text-align:left; width:100%;' border='0' class='centerTable'>";
    ...
    <button type="submit" name="btnViewToolDetails" class="clerkMainMenu">View Details</button>
	<?php echo $toolDetails ?>
	<p>________________________________________________________________________</p>

	<br>
	<br>
	<b><label class="pickup_label">Credit Card Number</label></b>
    <input type="text" name="ccNumber" class="pickup_input" />
    <label>eg. 1234567890123456</label>
    <br>
	<p></p>
    <b><label class="pickup_label">Expiration Date</label></b>
    <input type="date" name="expDate" class="pickup_input" />
	<label>eg. YYYY-MM</label>
    <br>

	<p>
    <button type="submit" style="width:150px;" name="btnCompletePickup" class="clerkMainMenu">Complete Pick-Up</button>
  </p>
	</form>
	<p>
